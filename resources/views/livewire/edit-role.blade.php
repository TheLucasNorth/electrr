<div>
    ...
    <form class="w-full" wire:submit.prevent="submitForm">
        ...
        <div class="md:flex flex-wrap md:items-center mb-6">
            ...
            <div class="md:w-full" wire:ignore>
                <textarea
                    ...
                    id="description" name="roleDescription">{{$description}}</textarea>
            </div>
        </div>
        ...
        @if($election->nominations)
            ...
            <div class="md:flex md:items-center mb-6">
                ...
                <div>
                    <input class="mr-2 leading-tight" type="checkbox" id="roleNominations" name="roleNominations"
                           wire:model.lazy="nominations" @unless(!$nominations) checked @endunless>
                </div>
            </div>
            ...
            @if($nominations)
                ...
                <div class="md:flex flex-wrap md:items-center mb-6">
                    <div>
                        <label class="block text-gray-500 font-bold md:text-right mb-1 md:mb-0 pr-4" for="seats">
                            You can ask candidates for specific information when nominating. Enter questions here,
                            separated
                            by semi-colons.
                        </label>
                    </div>
        ...
        <div
            class="md:flex flex-wrap md:items-center mb-6 @if(\Carbon\Carbon::parse($role->voting_open)->lte(\Carbon\Carbon::today())) border-red-500 border p-2 @endif ">
            @if(\Carbon\Carbon::parse($role->voting_open)->lte(\Carbon\Carbon::today()))
                <p class="w-full">Please be aware that changing this setting after voting has opened may cause your
